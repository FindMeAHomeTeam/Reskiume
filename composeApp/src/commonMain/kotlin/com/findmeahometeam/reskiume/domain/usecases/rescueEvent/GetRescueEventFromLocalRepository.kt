...
class GetRescueEventFromLocalRepository(private val localRescueEventRepository: LocalRescueEventRepository) {

    operator fun invoke(id: String): Flow<RescueEvent?> = flow {
        val result: RescueEvent? =
            localRescueEventRepository.getRescueEvent(id)
                ?.let { rescueEventWithAllNeedsAndNonHumanAnimalData: RescueEventWithAllNeedsAndNonHumanAnimalData ->
